    ...
    <div style={{ textAlign: "center", padding: "20px" }}>
      ...
      {response && (
        <div>
          ...
          {/* EcoBloom Carbon Impact Level */}
          {(() => {
            const total = Number(response?.footprint?.total || 0);
            const fallback = total < 10
              ? { level: "Low Carbon Impact", emoji: "🟢", message: "Your emissions are currently low. Keep making sustainable choices!" }
              : total <= 20
                ? { level: "Moderate Carbon Impact", emoji: "🟡", message: "Your footprint is moderate. A few changes can help reduce it further." }
                : { level: "High Carbon Impact", emoji: "🔴", message: "Your footprint is relatively high. Check your largest category for ways to improve." };
            const level = response?.footprint?.impactLevel || fallback.level;
            const emoji = response?.footprint?.impactKey === "low" ? "🟢" : response?.footprint?.impactKey === "moderate" ? "🟡" : response?.footprint?.impactKey === "high" ? "🔴" : fallback.emoji;
            const message = response?.footprint?.impactMessage || fallback.message;
            return (
              <div style={{ maxWidth: "720px", margin: "20px auto", padding: "22px", borderRadius: "18px", background: "linear-gradient(135deg, #f5fbf6, #eef8f0)", border: "1px solid #cfe5d3", boxShadow: "0 6px 18px rgba(0,0,0,0.08)" }}>
                <div style={{ fontSize: "18px", fontWeight: 700, color: "#4E6151" }}>{emoji} {level}</div>
                <div style={{ fontSize: "30px", fontWeight: 800, color: "#333", margin: "8px 0" }}>{total.toFixed(2)} kg CO₂</div>
